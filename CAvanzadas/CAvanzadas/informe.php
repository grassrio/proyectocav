...
session_start();
if (isset($_SESSION['usuario']) && isset($_POST['idLicitacion']) && isset($_POST['obrasInformar']) && isset($_POST['nombreInforme'])) {
...
$idObrasInformar = $_POST['obrasInformar'];
...
foreach($obrasInformarArray as $idCotizacion=>$obrasPorCotizacion){
    ...
    $nombreCotizacion = $rsCotizacion[Nombre];
    ...
    ->setCellValueByColumnAndRow(0,($filaHeaderInforme+1), $nombreCotizacion)
    ...
    for($i=0;$i<count($obrasPorCotizacion);$i++){
        ...
        if ($obra[Estado]=="Ejecutado" || $obra[Estado]=="Facturar 0,3"){
            ...
            //Vamos a marcar la obra como informada
            //informarObra($idObra,$nombreInforme);
    ...
    //Obtenemos el valor total informado para descontar de la licitacion
    $valorTotalObras = $objPHPExcel->getActiveSheet()->getCellByColumnAndRow($cantidadHeadersPreviosDinamico,($filaParaTotales+3))->getCalculatedValue();

    //Descuento el saldo de la licitacion
    $operacionDescontar = descontarLicitacion($idLicitacion,$valorTotalObras);
    //Si esto sale bien, informo cada obra
    if ($operacionDescontar==1){
        foreach($obrasPorCotizacion as $obra){
            $idObra = $obra[idObra];
            if ($obra[Estado]=="Ejecutado" || $obra[Estado]=="Facturar 0,3"){
                informarObra($idObra,$nombreInforme);
...
$objWriter->save("informes/".$nombreInforme.".xls");
